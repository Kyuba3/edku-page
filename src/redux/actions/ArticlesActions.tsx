...
export const addArticle = (articleData: Article) => {
  return (dispatch: Dispatch<ArticleActionTypes>) => {
    fetch('http://localhost:4000/api/articles', {
      method: 'POST',
      headers: {
    ...
    .then(article => dispatch({ type: ADD_ARTICLE_SUCCESS, payload: article }))
    .catch(error => dispatch({ type: ADD_ARTICLE_FAILURE, payload: error }));
  };
};
